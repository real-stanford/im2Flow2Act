...
class Pouring(TableTopUR5WSG50FinrayEnv):
    def __init__(
        self,
        **kwargs,
    ) -> None:
        super().__init__(**kwargs)
        self.task_description = "Pour water from the mug to the blue bowl."
    ...
    def setup_objs(self, world_model):
        ...
        mug_y_bound = [-0.1, 0.12]
        bowl_x_bound = [0.42, 0.65]
        bowl_y_bound = [0.15, 0.32]
        ...
        object_model = mjcf.from_path(
            os.path.normpath(
                os.path.join(
                    file_dir,
                    "../mujoco/asset/mujoco_scanned_objects/train/Down_To_Earth_Ceramic_Orchid_Pot_Asst_Blue/model.xml",
                )
            )
        )
        object_model.model = "blue_bowl"
        self.add_obj_from_model(
            world_model,
            object_model,
            target_pos,
        ...
        self.target_pos[1] = self.target_pos[1] - 0.125
    ...
    def check_success(self):
        ...
        reference_rotation_matrix = (
            self.mj_physics.data.site(reference_site).xmat.copy().reshape(3, 3)
        )
        euler_angles = R.from_matrix(reference_rotation_matrix).as_euler("xyz")
        bowl_indices = [15, 16, 17]
        target_pos = self.mj_physics.data.qpos.copy()[bowl_indices]
        if (
            np.abs(reference_position_vector[0] - target_pos[0]) < 0.07
            and euler_angles[0] < -np.pi / 3
        ):
            return True
